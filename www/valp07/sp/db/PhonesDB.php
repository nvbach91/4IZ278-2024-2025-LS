...
class PhonesDB extends Database
{
    ...
    public function deletePhone($productId)
    {
        $stmt = $this->pdo->prepare("DELETE FROM phones WHERE product_id = :id");
    ...
    public function createPhone($args)
    {
        $sql = "INSERT INTO $this->tableName (product_id, screen_size, ram, storage, battery)
            VALUES (:product_id, :screen_size, :ram, :storage, :battery)";
        $statement = $this->pdo->prepare($sql);
        $statement->execute([
            'product_id' => $args['product_id'],
            'screen_size' => $args['screen_size'],
