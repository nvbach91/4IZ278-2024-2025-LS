...
if (is_array($ids) && count($ids)) {
    ...
    $stmt = $db->prepare("SELECT * FROM cv07_goods WHERE id IN ($question_marks) ORDER BY name");
    # array values - setrepeme pole aby bylo indexovane od 0, jen kvuli dotazu, jinak neprojde
    $stmt->execute(array_values($ids));
    $goods = $stmt->fetchAll();
    
    
    $stmt_sum = $db->prepare("SELECT SUM(price) FROM cv07_goods WHERE id IN ($question_marks)");
    # array values - setrepeme pole aby bylo indexovane od 0, jen kvuli dotazu, jinak neprojde
