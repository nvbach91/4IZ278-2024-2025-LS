...
class ProductsDB extends Database
{
    ...
    public function EditItemById($id, $name, $price, $img, $category)
    {
        $sql = "UPDATE $this->tableName SET `name` = '$name', `price` = $price, `img` = '$img', `category_id` = $category WHERE `product_id` = $id;";
        $statement = $this->connection->prepare($sql);
        $statement->execute();
    }
}
