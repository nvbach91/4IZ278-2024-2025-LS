...
// oprava: musí být $stmt->execute, ne stmt->execute
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 3) uložit změny
    if ($action === 'save') {
        // pro shared user jen popis, stav a tagy
        ...
        if ($isOwner) {
            ...
            // shared user nemůže měnit titulek, ponecháme originál
            $newTitle = $title;
        }
    ...
    // 5) odebrat sdílení jiného uživatele (vlastník)
    if ($action === 'unshare' && $isOwner) {
        $unshareUserId = (int)($_POST['unshare_user_id'] ?? 0);
        if ($unshareUserId && $shareModel->unshare($taskId, $unshareUserId)) {
            $success[] = 'Sdílení bylo odebráno.';
        } else {
            $errors[] = 'Nepodařilo se odebrat sdílení.';
        }
        $sharedUsers = $shareModel->getSharedUsers($taskId);
    }
}
