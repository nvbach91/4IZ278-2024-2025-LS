...
{
    private $pageKey = 'myrecipes_page_filters';
    private $viewName = 'myrecipes';
    ...
    public function myRecipes()
    {
        ...
        if (empty($countFilters)) {
            $allRecipes = $recipeModel->getRecipesByOwnerId($user_id);
            // echo '<pre>' . print_r($allRecipes, true) . '</pre>';
            $totalRecipes = $allRecipes ? count($allRecipes) : 0;
        } else {
            $filteredRecipes = $recipeModel->getRecipesByOwnerIdAndFilters($user_id, $countFilters);
            $totalRecipes = $filteredRecipes ? count($filteredRecipes) : 0;
        }

        if (empty($_SESSION[$this->pageKey])) {
            // if no filters, get all recipes
            $recipes = $recipeModel->getRecipesByOwnerId($user_id, $recipesPerPage, $offset) ?? [];
        } else {
            $recipes = $recipeModel->getRecipesByOwnerIdAndFilters($user_id, $filters) ?? [];
        }
